...
for ii = 1:numBlocks
    dilationFactor = 2^(ii-1);
    
    layers = [
        convolution1dLayer(filterSize,numFilters,DilationFactor=dilationFactor,Padding="causal",Name="conv1_"+ii)
    ...
    lgraph = connectLayers(lgraph,outputName,"conv1_"+ii);
    ...
    if ii == 1
        ...
        lgraph = connectLayers(lgraph,"convSkip","add_" + ii + "/in2");
    else
        lgraph = connectLayers(lgraph,outputName,"add_" + ii + "/in2");
    end
...
xlabel('预测样本')
ylabel('预测结果')
string = {'QRTCN训练集预测结果对比'; ['RMSE = ' num2str(error1)]};
title(string)
xlim([1, M])
grid
...
xlabel('预测样本')
ylabel('预测结果')
string = {'QRTCN测试集预测结果对比'; ['RMSE = ' num2str(error2)]};
title(string)
xlim([1, N])
grid
